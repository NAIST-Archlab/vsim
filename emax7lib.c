...
struct dma_ctrl {
  ...
  /* Simple Mode */
  /* 0. MM2S_DMASR.Halted=0,Idle=1を確認 */
  ...
  /* 0. S2MM_DMASR.Halted=0,Idle=1を確認 */
  ...
  /* 4. MM2S_DMASR.bit4-6!=0ならエラー */
  /* 4. S2MM_DMASR.bit4-6!=0ならエラー */
  /* 4. MM2S_DMASR.IOC_Irqが1になるまで待機,1を書いてreset */
  /* 4. S2MM_DMASR.IOC_Irqが1になるまで待機,1を書いてreset */
};
...
printf("DR..DMA phase=%x i=%d j=%d dirty=%x\n", phase, i, j, emax6.lmmd[i][j]);
  ...
  /*旧正 ST/f=1 ⇒A   | ST/f=1 ×DR  A | ST/f=1 ×DR  A | ST/f=1 A⇒ ⇒B | ST/f=1 ×DR  B | ST/f=1 ×DR  B */
  /*新誤 ST/f=1 ⇒A   | ST/f=1 ×DR⇒A | ST/f=0 ×DR  A | ST/f=1 A⇒ ⇒B | ST/f=1 ×DR⇒B | ST/f=0 ×DR  B */
  /*                  |★stせずA初期化 |                |                |★stせずB初期化 |                */
  /*新正 ST/f=1 ⇒A   | ST/f=0 ×DR  A | ST/f=0 ×DR  A | ST/f=1 A⇒ ⇒B | ST/f=0 ×DR  B | ST/f=0 ×DR  B */
  /*新正 ST/f=1 ⇒A   |           A!=Bの場合自動drain有 | ST/f=1 A⇒ ⇒B                                   */
  /*新誤 ST/f=1 ⇒A   |           A==Bの場合自動drain無 | ST/f=1 ×DR⇒B                                   */
  /*                  |                                 |★stせずB初期化                                   */
  /*新正 ST/f=1 ⇒A   |           A==Bの場合手動drain A | ST/f=1     ⇒B                                   */

  /*改正 ST/f=1 ⇒A   | ST/f=1 A⇒ ⇒A | ST/f=0 ×DR  A | ST/f=1 A⇒ ⇒B | ST/f=1 B⇒ ⇒B | ST/f=0 ×DR  B */
  /*改正 ST/f=1 ⇒A   | ST/f=0 ×DR  A | ST/f=0 ×DR  A | ST/f=1 A⇒ ⇒B | ST/f=0 ×DR  B | ST/f=0 ×DR  B */
  /*改正 ST/f=1 ⇒A   |           A!=Bの場合自動drain有 | ST/f=1 A⇒ ⇒B                                   */
  /*改正 ST/f=1 ⇒A   |           A==Bの場合自動drain有 | ST/f=1 A⇒ ⇒B                                   */
...
//printf("load: adr=%08.8x len=%08.8x nxt=%08.8x\n", (Uint)dmadr, (Uint)dmlen, (Uint)dmnxt);
  ...
  if (mark) {
	...
	if (emax7[LANE].blkcount==0) {
	  emax7[LANE].lmmblktop = 0; /* ★★★未実装★★★ 先頭アドレスが0なので,addr_rangeに工夫が必要 */
	...
	if (emax7[LANE].lmmblklen==0)
	  ...
	  emax7[LANE].blkcount++; /* ★★★未実装★★★ continue 複数回のDMA起動が必要 */
      }
#endif
    }
#if 0
...
emax7_kick_dma(int LANE, int j) /* col */
{
  ...
  if (j != emax7[LANE].csel_save) {
    ((struct reg_ctrl*)emax7[LANE].reg_ctrl)->i[0].csel = j; /* DMA/LDDMQ/TRANS用に対象colセット */
...
exe(Uint op_ex1, Ull *d, Ull s1, Uint exp1, Ull s2, Uint exp2, Ull s3, Uint exp3, Uint op_ex2, Ull r4, Uint op_ex3, Ull r5)
{
  ...
  case OP_NOP:
    ex1_outd = r1;
    break;
  case OP_WHILE: /* emax7ncのlibとしては使用せず,bsim/emax7.cがsimlに使用 */
    t0 = (r1&0x00000000ffffffffLL)+(r2&0x00000000ffffffffLL);
    t0 &= 0x00000000ffffffffLL;
    ex1_outd = t0;
    if (t0==0) retval = 1;
    break;
  case OP_FOR: /* emax7ncのlibとしては使用せず,bsim/emax7.cがsimlに使用 */
    t0 = (r1&0x00000000ffffffffLL)+(r2&0x00000000ffffffffLL);
    t0 &= 0x00000000ffffffffLL;
    ex1_outd = t0;
    if (t0==0) retval = 1;
...
int emax7_unaligned_load_valid; /* mop(BR[][][1]adr+8); mop(BR[][][0]adr);連続指定を想定し,1の場合highが有効,0の場合無効  */
Ull emax7_unaligned_load_high;  /* mop(BR[][][1]adr+8); mop(BR[][][0]adr);連続指定を想定し,high側を一次保存しlow側にmerge */
